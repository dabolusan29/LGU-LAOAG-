...
function getNtpDaysRemaining(ntpDateStr) {
    if (!ntpDateStr) return null;
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const ntpDate = new Date(ntpDateStr);
    if (isNaN(ntpDate.getTime())) return null;
    
    // 1 month (30 days) allocated for the Awarding of NTP
    const ntpDeadlineDate = new Date(ntpDate.getTime());
    ntpDeadlineDate.setDate(ntpDeadlineDate.getDate() + 30);
    ntpDeadlineDate.setHours(0, 0, 0, 0);

    const timeDiff = ntpDeadlineDate.getTime() - today.getTime();
    return Math.ceil(timeDiff / (1000 * 60 * 60 * 24));
}

function checkNtpAwardingActiveWindow(ntpDateStr, status) {
    if (!ntpDateStr || status === "Completed") return false;

    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const ntpDate = new Date(ntpDateStr);
    if (isNaN(ntpDate.getTime())) return false;
    ntpDate.setHours(0, 0, 0, 0);

    const ntpDeadlineDate = new Date(ntpDate.getTime());
    ntpDeadlineDate.setDate(ntpDeadlineDate.getDate() + 30);
    ntpDeadlineDate.setHours(0, 0, 0, 0);

    // Active for 1 month starting from the date of NTP
    return today >= ntpDate && today <= ntpDeadlineDate;
}

function checkMaterialTestFifteenDayWindow(ntpDateStr, status) {
    if (!ntpDateStr || status === "Completed") return false;
    ...
    const materialTestDeadline = new Date(ntpDate.getTime());
    materialTestDeadline.setDate(materialTestDeadline.getDate() + 15);
    materialTestDeadline.setHours(0, 0, 0, 0);

    // Active for 15 days starting from the date of the award of the NTP
    return today >= ntpDate && today <= materialTestDeadline;
}
...
function renderTablePage() {
    ...
    paginatedItems.forEach(doc => {
        ...
        const isNtpAwardActive = checkNtpAwardingActiveWindow(doc.ntpDate, doc.status);
        const isMaterialTestActive = checkMaterialTestFifteenDayWindow(doc.ntpDate, doc.status);
        
        const rowClass = (isCompletionAlertActive || isNoaAlertActive || isNtpAwardActive || isMaterialTestActive) ? "table-danger" : "";
        ...
        if (isNtpAwardActive) {
            const ntpDaysLeft = getNtpDaysRemaining(doc.ntpDate);
            alertBadgesHtml += ` <span class="badge bg-info text-dark overdue-blink">📋 NTP Awarding (${ntpDaysLeft}d left)</span>`;
        }
        if (isMaterialTestActive) {
            alertBadgesHtml += ` <span class="badge bg-primary overdue-blink">🧪 Material Test Due</span>`;
        }
        if (isCompletionAlertActive) {
            alertBadgesHtml += ` <span class="badge bg-danger overdue-blink">⚠️ Awarding Window</span>`;
        }

        const materialTestNotice = isMaterialTestActive 
            ? `<div class="text-primary fw-bold small mt-1">🔬 Remarks: Request for Material Test Mandatory</div>` 
            : "";
...
function updateMetricsDashboard(docs) {
    ...
    const activeNtpAwardDocs = docs.filter(d => checkNtpAwardingActiveWindow(d.ntpDate, d.status));
    const activeMaterialTestDocs = docs.filter(d => checkMaterialTestFifteenDayWindow(d.ntpDate, d.status));
    
    const alertCount = activeAlertDocs.length + activeNoaDocs.length + activeNtpAwardDocs.length + activeMaterialTestDocs.length;
    ...
    // NOA Notifications
    // NOA Notifications (Color Updated to Purple Theme)
    activeNoaDocs.forEach(d => {
        ...
        // Dropdown menu panel notice tracking text alignment color
        notificationHtml += `<li class="text-dark mb-2 border-bottom pb-1" style="color: #6f42c1 !important;"><strong>[NO: ${d.projectNo || 'N/A'}] ${d.title}</strong><br>⏰ NOA Tracker Active (${daysLeft} days remaining until ${formattedLimitString})</li>`;
        
        // Dashboard main list style block modified to Bootstrap's standard purple/indigo context look if desired
        // Alternatively, use 'list-group-item-dark', 'list-group-item-secondary', etc.
        dashNotificationHtml += `<li class="list-group-item border-start border-3 my-1 rounded-1" style="background-color: #f3f0fa; border-left-color: #6f42c1 !important; color: #492780;"><strong>[NO: ${d.projectNo || 'N/A'}]<sup>NOA</sup> ${d.title}</strong> — ⏰ Timeline Window (${daysLeft} days remaining until ${formattedLimitString})</li>`;
    });

    // corrected: NTP Awarding (1 Month) Notifications
    activeNtpAwardDocs.forEach(d => {
        const ntpDaysLeft = getNtpDaysRemaining(d.ntpDate);
        const targetNtpDate = new Date(d.ntpDate);
        targetNtpDate.setDate(targetNtpDate.getDate() + 30);
        const formattedNtpLimitString = formatUIDate(targetNtpDate);

        notificationHtml += `<li class="text-info mb-2 border-bottom pb-1"><strong>[NO: ${d.projectNo || 'N/A'}] ${d.title}</strong><br>📋 NTP Awarding Active Status Monitor (${ntpDaysLeft} days remaining)</li>`;
        dashNotificationHtml += `<li class="list-group-item list-group-item-info border-start border-info border-3 my-1 rounded-1"><strong>[NO: ${d.projectNo || 'N/A'}]<sup>NTP Award</sup> ${d.title}</strong> — 📋 1-Month Active Award Tracking (${ntpDaysLeft} days remaining until ${formattedNtpLimitString})</li>`;
    });

    // corrected: Request for Material Test (15 Days) Notifications
    activeMaterialTestDocs.forEach(d => {
        const targetMtDate = new Date(d.ntpDate);
        targetMtDate.setDate(targetMtDate.getDate() + 15);
        const formattedMtLimitString = formatUIDate(targetMtDate);

        notificationHtml += `<li class="text-primary mb-2 border-bottom pb-1"><strong>[NO: ${d.projectNo || 'N/A'}] ${d.title}</strong><br>🧪 Material Test Alert Phase active (Deadline: ${formattedMtLimitString})</li>`;
        dashNotificationHtml += `<li class="list-group-item list-group-item-primary border-start border-primary border-3 my-1 rounded-1"><strong>[NO: ${d.projectNo || 'N/A'}]<sup>Material Test</sup> ${d.title}</strong> — 🧪 15-Day Phase: Request for Material Test Required by ${formattedMtLimitString}</li>`;
    });

    // Completion Window Notifications
